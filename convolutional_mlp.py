...
Usage:
    ...
    Default train False
            predict False
            dataset: "./skim_data_convnet_target0.pkl.gz"
            N epochs: 1000
            batch size: 500
            rate: 0.01
...
from mlp_1h import HiddenLayer
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200, dataset='mnist.pkl.gz',
    ...
    x = T.tensor4('x')      # data is rasterized images
    y = T.ivector('y')      # labels are 1D [0,1...5]

    print("... building the model")

    # Reshape matrix of rasterized images of shape (batch_size, 22*50) to a
    # 4D tensor, compatible with the LeNetConvPoolLayer. (22, 50) is the size
    # of the MINERvA images, and there are 3 views
    layer0_input = x.reshape((batch_size, 3, 22, 50))
    ...
    # so 4D Tensor output is of shape (batch_size, nkerns[0], 10, 24)
    filt_h = 3
    filt_w = 3
    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 3, 22, 50),
        filter_shape=(nkerns[0], 3, filt_h, filt_w),
        poolsize=(2, 2)
    )

    # Construct the second convolutional pooling layer; filtering reduces the
    # image size to (10 - 3 + 1, 24 - 3 + 1) = (8, 22) and maxpooling further
    # reduces it to (8 / 2, 22 / 2) = (4, 11); 4D output tensor therefore has
    # shape (batch_size, nkerns[1], 4, 11)
    layer1 = LeNetConvPoolLayer(
        rng,
        input=layer0.output,
        image_shape=(batch_size, nkerns[0], 10, 24),
        filter_shape=(nkerns[1], nkerns[0], filt_h, filt_w),
        poolsize=(2, 2)
    )
    ...
    # This generates a martix of shape (batch_size, nkerns[1] * 4 * 11),
    # or whatever the output of layer 1 is.
...
def predict(dataset, nkerns=[20, 50]):
    ...
    layer0_input = x.reshape((samp_size, 3, 22, 50))
    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(samp_size, 3, 22, 50),
        filter_shape=(nkerns[0], 3, filt_h, filt_w),
        poolsize=(2, 2),
        W=pars[6],
        b=pars[7]
    ...
    layer1 = LeNetConvPoolLayer(
        rng,
        input=layer0.output,
        image_shape=(samp_size, nkerns[0], 10, 24),
        filter_shape=(nkerns[1], nkerns[0], filt_h, filt_w),
        poolsize=(2, 2),
        W=pars[4],
        b=pars[5]
    ...
    predicted_values = \
        predict_model(test_set_x[:samp_size].reshape(samp_size, 3, 22, 50))
