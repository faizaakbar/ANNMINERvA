#!/usr/bin/env python
"""
This is a "tricolumnar" network operating on Minerva X, U, V, with a muon-data
vector included as a "wide" input in the fully-connected portion of the network.
It assumes an HDF5 input file with groups organized like so:
...
Execution:
    python minerva_vtx_zeta.py -h / --help
...
from minerva_ann_networks import build_network_zeta
...
def get_theano_input_tensors():
    ...
    input_var_mu = T.matrix('inputs')
    return [input_var_x, input_var_u, input_var_v, input_var_mu]
...
def get_hits(data):
    ...
    """
    inputu, inputv, inputx, inputmu = data[1], data[2], data[3], data[4]
    inputs = [inputx, inputu, inputv, inputmu]
    return inputs
...
def get_hits_and_targets(data):
    ...
    """
    inputu, inputv, inputx, inputmu = data[1], data[2], data[3], data[4]
    inputs = [inputx, inputu, inputv, inputmu, data[6]]
    return inputs
...
if __name__ == '__main__':
    ...
    import logging
    logfilename = options.logfilename or 'minerva_vtx_zeta.log'
    logging.basicConfig(
    ...
    build_network_function = build_network_zeta
